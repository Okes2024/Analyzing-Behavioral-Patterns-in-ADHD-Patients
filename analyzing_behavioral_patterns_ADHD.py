...
def generate_synthetic_data(n=600, seed=42):
    ...
    # Controls centered near 0, ADHD shifted positive
    ...
    # Reaction time and variability (higher and more variable in ADHD)
    reaction_time_ms = (
    ...
    # Sleep & activity (actigraphy proxy)
    sleep_quality = (
...
def describe_by_group(df, features):
    ...
    for feat in features:
        ...
        rows.append({
            "feature": feat,
            "ADHD_mean": np.mean(a),
            "ADHD_sd": np.std(a, ddof=1),
            "Control_mean": np.mean(c),
            "Control_sd": np.std(c, ddof=1),
            "Cohens_d_ADHD_minus_Control": d
        })
...
def pca_scatter(df, features):
    ...
    plt.suptitle(f"Explained variance: PC1={exp[0]:.2f}, PC2={exp[1]:.2f}", y=0.98, fontsize=9)
    plt.legend()
    plt.tight_layout()
...
def main():
    ...
    print("\n=== Descriptive Stats & Effect Sizes (ADHD - Control) ===")
    print(desc.to_string(index=False, float_format=lambda x: f"{x:0.3f}"))
    ...
    print("\n=== KMeans Clustering Crosstab (true group vs. cluster) ===")
    print(table)
    ...
    if not args.no-plots:
        ...
        # Optional histograms for a few key features
